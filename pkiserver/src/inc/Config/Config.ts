...
import {ConfigOptionsPkiServer, ENV_DUTY_DB, ENV_OPTIONAL_DB, SchemaConfigOptionsPkiServer} from 'flyingfish_schemas';
 ...
 * Config
 */
export class Config extends ConfigCore<ConfigOptionsPkiServer> {
    ...
    public static readonly DEFAULT_PKISERVER_PORT = 5334;
    ...
    public static override getInstance(): Config {
        if (!ConfigCore._instance) {
            const instance = new Config(SchemaConfigOptionsPkiServer);
     ...
     * @protected
     */
    protected override _loadEnv(aConfig: ConfigOptionsPkiServer | null): ConfigOptionsPkiServer | null {
        let config = aConfig;
        ...
        if (config) {
            ...
            config = {
                ...
                pkiserver: {
                    port: Config.DEFAULT_PKISERVER_PORT
                }
            };
        }
        ...
        if (process.env[ENV_OPTIONAL.PKISERVER_PORT]) {
            if (!config.pkiserver) {
                config.pkiserver = {};
            }

            config.pkiserver.port = parseInt(process.env[ENV_OPTIONAL.PKISERVER_PORT]!, 10) ||
                Config.DEFAULT_PKISERVER_PORT;
        }

        // pki organization --------------------------------------------------------------------------------------------

        if (process.env[ENV_OPTIONAL.PKISERVER_ORGANIZATION]) {
            if (!config.pkiserver) {
                config.pkiserver = {};
            }
     ...
     * @protected
     */
    protected override _setDefaults(config: ConfigOptionsPkiServer | null): ConfigOptionsPkiServer | null {
        if (config === null) {
            return null;
        }
