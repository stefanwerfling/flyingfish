...
import {ConfigOptionsLan, SchemaConfigOptionsLan} from 'flyingfish_schemas';
...
export enum ENV_OPTIONAL {
    ...
    LAN_RECONCILE_INTERVAL_MS = 'FLYINGFISH_LAN_RECONCILE_INTERVAL_MS',
...
/**
 * Config — the LAN DHCP/RA-server part is database-free; its config needs the Hub registry
 * (to register + pull the WAN interface + report the lease), the node PKI block and a
 * reconcile interval.
 */
export class Config extends ConfigCore<ConfigOptionsLan> {
    ...
    public static override getInstance(): Config {
        if (!ConfigCore._instance) {
            const instance = new Config(SchemaConfigOptionsLan);
     ...
     * @protected
     */
    protected override _loadEnv(aConfig: ConfigOptionsLan | null): ConfigOptionsLan | null {
        let config = aConfig;
        ...
        if (process.env[ENV_OPTIONAL.LAN_RECONCILE_INTERVAL_MS]) {
            if (!config.lan) {
                config.lan = {};
            }

            config.lan.reconcileIntervalMs = parseInt(process.env[ENV_OPTIONAL.LAN_RECONCILE_INTERVAL_MS]!, 10) ||
        ...
        // Hub registry (v2): both url and secret required to register + report.
     ...
     * @protected
     */
    protected override _setDefaults(config: ConfigOptionsLan | null): ConfigOptionsLan | null {
        if (config === null) {
            return null;
        }
