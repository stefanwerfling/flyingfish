...
export const SchemaBackendConfigOptions = SchemaConfigOptions.extend({
    ...
    // Retained for backward compatibility with existing config files only:
    // HimHIP was folded into the netdevice part (host facts now reported over
    // HTTP) and the standalone himhip container removed, so nothing reads
    // use/secret anymore. Vts rejects unknown keys, so the field stays optional
    // to keep older config.json files valid; it is never populated.
