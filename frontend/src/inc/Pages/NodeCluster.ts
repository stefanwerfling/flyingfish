...
export class NodeCluster extends BasePage {
    ...
    protected _join(mount: JQuery): void {
        ...
        const btn = jQuery('<button style="margin:4px 0 2px;padding:7px 14px;border:0;border-radius:6px;background:#0f8a8a;color:#fff;cursor:pointer;font-weight:600">Generate join package</button>').appendTo(mount);
        ...
        btn.on('click', async(): Promise<void> => {
            ...
            btn.prop('disabled', false).text('Generate join package');
        });
    }
    ...
    protected _joinCluster(mount: JQuery): void {
        ...
        const input = jQuery('<textarea rows="3" placeholder=\'{"meshHost":"…","meshPort":5336,"bootstrapToken":"…","caFingerprint":"…"}\' style="width:100%;box-sizing:border-box;margin-top:4px;padding:8px 10px;border-radius:6px;border:1px solid rgba(127,127,127,.35);background:rgba(127,127,127,.08);font-family:monospace;font-size:12px"></textarea>').appendTo(mount);
        const btn = jQuery('<button style="margin-top:6px;padding:7px 14px;border:0;border-radius:6px;background:#0f8a8a;color:#fff;cursor:pointer;font-weight:600">Apply join package</button>').appendTo(mount);
        ...
        btn.on('click', async(): Promise<void> => {
            ...
            try {
                parsed = JSON.parse(String(input.val()));
            } catch (e) {
            ...
            btn.prop('disabled', false).text('Apply join package');
        });
    }
