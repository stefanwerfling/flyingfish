 ...
 */
const makeNode = async(name: string): Promise<JoinNode> => {
...
describe('one-port cross-Hub join bootstrap (cluster/mesh 9.5.12.2 model b)', () => {
    let nodeA: JoinNode;
    let nodeB: JoinNode;

    beforeAll(async() => {
        nodeA = await makeNode('node-a');
        nodeB = await makeNode('node-b');
    });
...
describe('one-port join bootstrap rejects an invalid token (cluster/mesh 9.5.12.2)', () => {
    ...
    beforeAll(async() => {
        nodeA = await makeNode('reject-a');
        nodeB = await makeNode('reject-b');
    });
