...
describe('ClusterLocalStateProvider', () => {
    test('publishes a hub descriptor plus a domain:<id> summary per domain', async() => {
        const provider = new ClusterLocalStateProvider(async() => [
            {id: 1, domainname: 'a.test', disable: false, fixdomain: true, recordless: false, parent_id: 0},
            {id: 2, domainname: 'b.test', disable: true, fixdomain: false, recordless: true, parent_id: 1}
        ]);
        ...
        expect(entries[2].value).toEqual({
            id: 2, name: 'b.test', priority: 0, disable: true, fix: false, recordless: true, parentId: 1
        });
    });
