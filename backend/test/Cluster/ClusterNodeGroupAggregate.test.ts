/**
 * Tests for the cluster-global node-group projection (Cluster/Mesh epic 9.5.12.3): the
 * groups + memberships aggregated from the gossip under their global, cluster-stable UUID
 * keys — one shared set. Verifies groups and members are collected, per-node/hub/rbac
 * entries are ignored, malformed values are dropped, and the `node_group:` /
 * `node_group_member:` key prefixes do not collide (the member prefix must not be
 * mis-parsed as a group). Pure/data-level, network-free.
 */
import {
