...
import {Map as NginxMap} from './Config/Map.js';
import {Server as NginxConfServer, ServerXFrameOptions} from './Config/Server.js';
import {Upstream, UpstreamLoadBalancingAlgorithm} from './Config/Upstream.js';
import {NginxHTTPVariables} from './NginxHTTPVariables.js';
import {NginxLogFormatJson} from './NginxLogFormatJson.js';
...
/**
 * Builds the nginx configuration from the database into the shared NginxConfig.
 * Extracted from NginxService as the first structural split (phase 2);
 * behaviour-preserving (verified by the config characterization snapshots).
 */
export class NginxConfigBuilder {
    ...
    public async build(syslog: SysLogServer | null, control: NginxControlHttpServer | null): Promise<void> {
        ...
        this._addServerListens(
            ...
            NginxListenProtocol.tcp,
            {
                network: {
                    ...
                    ip: NginxConfigBuilder.DEFAULT_IP_LOCAL
                }
            },
            {
                network: {
                    port: this._proxyUpstreamServer,
                    ip: NginxConfigBuilder.DEFAULT_IP6_PUBLIC
                }
            },
            false,
            false,
        ...
        tSshInternUpstreamProxy.addServer({
            address: NginxConfigBuilder.DEFAULT_IP_LOCAL,
            port: this._proxyUpstreamServer,
            weight: 0,
            max_fails: 0,
            fail_timeout: 0
        ...
        for (const listenPort of streamMap.keys()) {
                            ...
                            if (streamCollect.upstreams.length > 0) {
                                for (const tupstream of streamCollect.upstreams) {
                                    ...
                                    if (tupstream.proxy_protocol_out) {
                                        ...
                                        destination_address = NginxConfigBuilder.DEFAULT_IP_LOCAL;
                                ...
                                case NginxStreamSshR.out:
                                    if (streamCollect.sshport) {
                                        ...
                                        if (streamCollects.listen.proxy_protocol) {
                                            ...
                                            destination_address_out = NginxConfigBuilder.DEFAULT_IP_LOCAL;
            ...
            if (syslog && syslog.isRunning()) {
                aServer.addVariable(
                    'access_log',
                    `syslog:server=${syslog.getOptions().address}:${syslog.getOptions().port},` +
                    `tag=${NginxConfigBuilder.SYSLOG_TAG} ` +
        ...
        for await (const listenPort of httpMap.keys()) {
            ...
            for await (const domainName of domainHttps.domains.keys()) {
                ...
                if (domainName !== NginxConfigBuilder.DEFAULT_DOMAIN_NAME) {
        ...
        if (statusListen) {
            ...
            const locStatus = new Location(NginxConfigBuilder.LOCATION_STATUS);
            locStatus.addVariable('stub_status', 'on');
            locStatus.addVariable('access_log', 'off');
            locStatus.addVariable('allow', NginxConfigBuilder.DEFAULT_IP_LOCAL);
